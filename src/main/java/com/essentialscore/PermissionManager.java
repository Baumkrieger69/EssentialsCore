...
public class PermissionManager {
    ...
    public PermissionManager(ApiCore apiCore) {
        ...
        this.console = new ConsoleFormatter(
            apiCore.getLogger(),
            apiCore.getConfig().getString("console.prefixes.permission-manager", "&8[&5&lPermissionManager&8]"),
            ...
            if (apiCore.getPermissionCacheSize() > 0) {
                ...
                try {
                    ...
                    if (permExactCache instanceof Map) {
                        ((Map<String, Boolean>) permExactCache).remove(cacheKey);
                    }
                } catch (Exception e) {
    ...
    public java.util.Set<String> getAllRegisteredPermissions() {
        ...
        if (permissionsHooked && permissionsProvider != null) {
            try {
                ...
                try {
                    // Nach registrierten Knoten suchen
